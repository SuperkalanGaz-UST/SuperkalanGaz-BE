...
import { CsatService, CsatSummary, RatingListItem } from './csat.service';
...
/**
 * CSAT Feedback & Analytics (journey BM-US-08). Following up on low-rated
 * deliveries is Branch Manager day-to-day ops (AGENTS.md §7) — the FA reads CSAT
 * cross-branch (FA-US-04) but does not resolve, so only BM reaches these
 * handlers. Scope comes from the verified Principal, never the client.
 *
 * There is deliberately no POST /csat/ratings: ratings are submitted by customers
 * on mobile, not created through the CRM.
 */
@Controller('csat')
